...
RSpec.describe Direwolf do
  ...
  it 'goes back to hunting white walkers if it leaves all starks' do
    summer_wolf = Direwolf.new('Summer', 'Winterfell')
    expect(summer_wolf.hunts_white_walkers?).to be true

    arya_stark = Stark.new('Arya')
    summer_wolf.protects(arya_stark)

    expect(summer_wolf.hunts_white_walkers?).to be false
